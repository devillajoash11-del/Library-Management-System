...
public class LibraryManagementSystem extends JFrame {
    private static final Color STUDENT_BACKGROUND = new Color(216, 245, 242);
    private static final Color ADMIN_BACKGROUND = new Color(216, 245, 242);

    public static void main(String[] args) {
        javax.swing.SwingUtilities.invokeLater(LibraryManagementSystem::new);
    }
    ...
    private final Map<Book, List<JLabel>> bookAvailabilityLabels = new HashMap<>();
    ...
    public LibraryManagementSystem() {
        ...
        outputArea.setBorder(BorderFactory.createTitledBorder("Information Panel"));
    ...
    private void initializeData() {
        ...
        reports.add(new Report("R001", "Borrow Summary", "Total active borrow transactions: 0", new Date()));
    ...
    private JPanel createLoginPanel() {
        JPanel panel = new JPanel(new BorderLayout()) {
            private final Image backgroundImage = loadLoginBackground();
            ...
            protected void paintComponent(Graphics graphics) {
                ...
                if (backgroundImage != null) {
                    ...
                    int imageWidth = (int) (backgroundImage.getWidth(null) * scale);
                    int imageHeight = (int) (backgroundImage.getHeight(null) * scale);
                    int imageX = (getWidth() - imageWidth) / 2;
                    int imageY = (getHeight() - imageHeight) / 2;
                    graphics2D.drawImage(backgroundImage, imageX, imageY, imageWidth, imageHeight, null);
                    graphics2D.setColor(new Color(10, 25, 35, 125));
                    graphics2D.fillRect(0, 0, getWidth(), getHeight());
                } else {
                    graphics2D.setColor(new Color(216, 245, 242));
                    graphics2D.fillRect(0, 0, getWidth(), getHeight());
                }
                graphics2D.dispose();
            }
        };
        panel.setBorder(BorderFactory.createEmptyBorder(38, 180, 38, 180));

        JPanel formPanel = new JPanel(new GridLayout(0, 2, 12, 14));
        formPanel.setOpaque(false);
        formPanel.setBorder(BorderFactory.createEmptyBorder(22, 24, 22, 24));
        ...
        JToggleButton viewPasswordButton = new JToggleButton(new EyeIcon(false));
        char passwordEchoChar = loginPasswordField.getEchoChar();
        ...
        loginIDField.setBackground(Color.WHITE);
        ...
        viewPasswordButton.setBackground(new Color(20, 48, 89));
        viewPasswordButton.setForeground(Color.WHITE);
        viewPasswordButton.setBorder(BorderFactory.createEmptyBorder(8, 10, 8, 10));
        viewPasswordButton.setToolTipText("View Password");

        loginButton.addActionListener(e -> handleLogin());
        viewPasswordButton.addActionListener(e -> {
            if (viewPasswordButton.isSelected()) {
                loginPasswordField.setEchoChar((char) 0);
                viewPasswordButton.setIcon(new EyeIcon(true));
                viewPasswordButton.setToolTipText("Hide Password");
            } else {
                loginPasswordField.setEchoChar(passwordEchoChar);
                viewPasswordButton.setIcon(new EyeIcon(false));
                viewPasswordButton.setToolTipText("View Password");
            }
        });
    ...
    private Image loadStudentBackground() {
        try {
            URL imageUrl = URI.create(
                "https://images.unsplash.com/photo-1521587760476-6c12a4b040da?auto=format&fit=crop&w=1600&q=85"
            ).toURL();
            return new ImageIcon(imageUrl).getImage();
        } catch (RuntimeException | java.io.IOException exception) {
            return null;
    ...
    private Image loadAdminBackground() {
        try {
            URL imageUrl = URI.create(
    ...
    private JPanel createStudentPanel() {
        JPanel panel = new JPanel(new CardLayout()) {
            private final Image backgroundImage = loadStudentBackground();
            ...
            protected void paintComponent(Graphics graphics) {
                ...
                if (backgroundImage != null) {
                    ...
                    int imageY = (getHeight() - imageHeight) / 2;
                    graphics2D.drawImage(backgroundImage, imageX, imageY, imageWidth, imageHeight, null);
                    graphics2D.setColor(new Color(18, 31, 31, 108));
                    graphics2D.fillRect(0, 0, getWidth(), getHeight());
                    java.awt.GradientPaint shade = new java.awt.GradientPaint(
                        0, 0, new Color(12, 29, 34, 118),
                        0, getHeight(), new Color(12, 29, 34, 35)
                    );
                    graphics2D.setPaint(shade);
                    graphics2D.fillRect(0, 0, getWidth(), getHeight());
                } else {
                    graphics2D.setColor(STUDENT_BACKGROUND);
                    graphics2D.fillRect(0, 0, getWidth(), getHeight());
                }
                graphics2D.dispose();
            }
        ...
        buttonPanel.setBorder(BorderFactory.createEmptyBorder(22, 22, 22, 22));
        buttonPanel.setOpaque(false);

        JLabel dashboardLabel = new JLabel("Student Dashboard", SwingConstants.CENTER);
        dashboardLabel.setFont(new Font("SansSerif", Font.BOLD, 26));
        dashboardLabel.setForeground(Color.WHITE);
        dashboardLabel.setBorder(BorderFactory.createEmptyBorder(4, 0, 18, 0));
        buttonPanel.add(dashboardLabel, BorderLayout.NORTH);
        ...
        styleButton(logoutButton, new Color(170, 62, 62));

        Color studentButtonColor = new Color(23, 54, 62);
        viewBooksButton.setBackground(studentButtonColor);
        searchBooksButton.setBackground(studentButtonColor);
        borrowBookButton.setBackground(studentButtonColor);
        recordsButton.setBackground(studentButtonColor);
        returnBookButton.setBackground(studentButtonColor);

        // Pinapalaki ang pangunahing student buttons para madaling gamitin.
        java.awt.Dimension largeBtn = new java.awt.Dimension(320, 72);
        ...
        Dimension compactLogoutBtn = new Dimension(140, 38);
        logoutButton.setPreferredSize(compactLogoutBtn);
        logoutButton.setMinimumSize(compactLogoutBtn);
        logoutButton.setMaximumSize(compactLogoutBtn);
        viewBooksButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        searchBooksButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        borrowBookButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        recordsButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        returnBookButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        logoutButton.setFont(new Font("SansSerif", Font.BOLD, 13));

        viewBooksButton.addActionListener(e -> {
            refreshBookAvailabilityLabels();
        ...
        buttonPanel.add(logoutPanel, BorderLayout.SOUTH);
    ...
    private JPanel createLibrarianPanel() {
        ...
        Dimension compactLogoutBtnLib = new Dimension(130, 36);
        logoutButton.setPreferredSize(compactLogoutBtnLib);
        logoutButton.setMinimumSize(compactLogoutBtnLib);
        logoutButton.setMaximumSize(compactLogoutBtnLib);
    ...
    private JPanel createAdminPanel() {
        JPanel panel = new JPanel(new CardLayout()) {
            private final Image backgroundImage = loadAdminBackground();
            ...
            protected void paintComponent(Graphics graphics) {
                ...
                if (backgroundImage != null) {
                    ...
                    graphics2D.setColor(ADMIN_BACKGROUND);
                    graphics2D.fillRect(0, 0, getWidth(), getHeight());
                }
                graphics2D.dispose();
            }
        ...
        JLabel dashboardLabel = new JLabel("Administrator Dashboard", SwingConstants.CENTER);
        dashboardLabel.setFont(new Font("SansSerif", Font.BOLD, 26));
        dashboardLabel.setForeground(Color.WHITE);
        dashboardLabel.setBorder(BorderFactory.createEmptyBorder(4, 0, 18, 0));
        buttonPanel.add(dashboardLabel, BorderLayout.NORTH);
        ...
        styleButton(deleteBookButton, new Color(170, 62, 62));
        styleButton(searchBookButton);
        styleButton(approveRequestButton);
        styleButton(rejectRequestButton, new Color(170, 62, 62));
        styleButton(recordsButton);
        styleButton(logoutButton, new Color(170, 62, 62));

        Color adminButtonColor = new Color(72, 42, 25);
        addBookButton.setBackground(adminButtonColor);
        searchBookButton.setBackground(adminButtonColor);
        approveRequestButton.setBackground(adminButtonColor);
        recordsButton.setBackground(adminButtonColor);

        // Pinapalaki ang mahahalagang administrator buttons.
        java.awt.Dimension largeBtnAdmin = new java.awt.Dimension(320, 72);
        ...
        approveRequestButton.setPreferredSize(largeBtnAdmin);
        approveRequestButton.setMinimumSize(largeBtnAdmin);
        approveRequestButton.setMaximumSize(largeBtnAdmin);
        rejectRequestButton.setPreferredSize(largeBtnAdmin);
        rejectRequestButton.setMinimumSize(largeBtnAdmin);
        rejectRequestButton.setMaximumSize(largeBtnAdmin);
        ...
        Dimension compactLogoutBtnAdmin = new Dimension(140, 38);
        logoutButton.setPreferredSize(compactLogoutBtnAdmin);
        logoutButton.setMinimumSize(compactLogoutBtnAdmin);
        logoutButton.setMaximumSize(compactLogoutBtnAdmin);
        addBookButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        deleteBookButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        searchBookButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        approveRequestButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        rejectRequestButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        recordsButton.setFont(new Font("SansSerif", Font.BOLD, 16));
        logoutButton.setFont(new Font("SansSerif", Font.BOLD, 13));
        ...
        ((CardLayout) panel.getLayout()).show(panel, "buttons");
    ...
    private JPanel createAdminRecordsPanel(Runnable backAction) {
        ...
        String[] columns = {"Request ID", "Book Name", "Borrower Full Name", "Borrowed At", "Returned At", "Request Status", "Borrow Status"};
        ...
        for (BorrowRequest request : requests) {
            ...
            String borrowerName = request.getBorrowerName().isEmpty()
                ? borrower == null ? request.getStudentID() : borrower.getName()
                : request.getBorrowerName();
            BorrowTransaction transaction = findTransaction(request.getStudentID(), request.getBookID());
            String borrowedAt = transaction == null ? "-" : formatDate(transaction.getBorrowDate());
            String returnedAt = transaction == null || transaction.getReturnDate() == null
                ? "Not yet returned" : formatDate(transaction.getReturnDate());
            String borrowStatus = transaction == null ? "-" : transaction.getStatus();
            model.addRow(new Object[]{request.getRequestID(), bookName, borrowerName, borrowedAt,
                returnedAt, request.getStatus(), borrowStatus});
        }

        for (BorrowTransaction transaction : transactions) {
            boolean hasRequest = false;
            for (BorrowRequest request : requests) {
            if (request.getStudentID().equalsIgnoreCase(transaction.getBorrowerID())
                && request.getBookID().equalsIgnoreCase(transaction.getBookID())) {
                hasRequest = true;
                break;
            }
            }
            ...
            model.addRow(new Object[]{"-", bookName, borrowerName, formatDate(transaction.getBorrowDate()),
                returnedAt, "No request record", transaction.getStatus()});
        }
        ...
        recordsTable.setSelectionBackground(new Color(80, 140, 180));
        recordsTable.setSelectionForeground(Color.WHITE);
        recordsTable.getTableHeader().setFont(new Font("SansSerif", Font.BOLD, 12));
        recordsTable.getTableHeader().setBackground(new Color(20, 48, 89));
    ...
    // Hinahanap ang account gamit ang user ID at ipinapakita ang nakarehistrong password.
    private void showForgotPasswordDialog() {
        ...
        if (user == null) {
            JOptionPane.showMessageDialog(this, "No account was found for that user ID.",
    ...
    private void handleLogin() {
        ...
        if (user instanceof Student) {
            checkStudentOverdueNotifications((Student) user);
        }
    ...
    private void showAddBookDialog() {
        ...
        form.setPreferredSize(new Dimension(390, 195));
        ...
        books.add(new Book(bookID, title, author, "", "", true, coverField.getText().trim()));
        ...
        for (Book book : books) {
            if (book.getBookID().toLowerCase().contains(searchTerm)
                ...
                result.append("Book: ").append(book.getTitle())
                        ...
                        .append("\nStatus: ").append(book.isAvailable() ? "Available" : "Occupied")
                        .append("\n\n");
                matchCount++;
            }
        }
    ...
    private JPanel createBookCard(Book book) {
        ...
        statusLabel.setBorder(BorderFactory.createEmptyBorder(5, 4, 2, 4));
        updateBookAvailabilityLabel(book, statusLabel);
        bookAvailabilityLabels.computeIfAbsent(book, ignored -> new ArrayList<>()).add(statusLabel);
    ...
    private void updateBookAvailabilityLabel(Book book, JLabel statusLabel) {
        boolean available = book.isAvailable() && !isBookOccupied(book.getBookID());
        statusLabel.setText(available ? "AVAILABLE" : "OCCUPIED");
        statusLabel.setForeground(available ? new Color(46, 125, 50) : new Color(198, 40, 40));
    }

    private void refreshBookAvailabilityLabels() {
        for (Map.Entry<Book, List<JLabel>> entry : bookAvailabilityLabels.entrySet()) {
            for (JLabel statusLabel : entry.getValue()) {
                updateBookAvailabilityLabel(entry.getKey(), statusLabel);
            }
        }
    }

    private boolean isBookOccupied(String bookID) {
        for (BorrowRequest request : requests) {
            if (request.getBookID().equalsIgnoreCase(bookID)
                    && ("Pending".equalsIgnoreCase(request.getStatus())
                    || "Successful".equalsIgnoreCase(request.getStatus()))) {
                return true;
            }
        }
        for (BorrowTransaction transaction : transactions) {
    ...
    private static class EyeIcon implements Icon {
        ...
        public void paintIcon(java.awt.Component component, Graphics graphics, int x, int y) {
            ...
            g2.setColor(new Color(55, 65, 75));
            g2.setStroke(new BasicStroke(1.8f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
            g2.drawOval(x + 2, y + 4, 18, 10);
            g2.fillOval(x + 9, y + 7, 4, 4);
            if (!visible) {
                g2.drawLine(x + 3, y + 2, x + 19, y + 16);
            }
            g2.dispose();
        }
    }
    ...
    private void styleButton(JButton btn) {
        ...
        btn.setBackground(new Color(20, 48, 89));
    ...
    private String getBorrowHistory() {
        ...
        for (BorrowRequest request : requests) {
            if (request.getStudentID().equals(currentUser.getUserID())) {
                ...
                BorrowTransaction transaction = findTransaction(request.getStudentID(), request.getBookID());
                String borrowerName = request.getBorrowerName().isEmpty()
                    ? currentUser.getName() : request.getBorrowerName();
                String borrowedAt = transaction == null ? "Pending approval"
                    : formatDate(transaction.getBorrowDate());
                String returnedAt = transaction == null || transaction.getReturnDate() == null
                    ? "Not yet returned" : formatDate(transaction.getReturnDate());
                builder.append("Book: ").append(bookName)
                    .append("\nBorrower: ").append(borrowerName)
                    .append("\nRequest ID: ").append(request.getRequestID())
                    .append("\nBorrowed at: ").append(borrowedAt)
                    .append("\nReturned at: ").append(returnedAt)
                    .append("\nStatus: ").append(transaction == null ? request.getStatus() : "Successful")
                    .append("\n\n");
            }
        }
    ...
    private JPanel createBorrowRecordsPanel(Runnable backAction) {
        ...
        String[] columns = {"Book Name", "Full Name", "Borrowed At", "Return By", "Returned At", "Status"};
        DefaultTableModel model = new DefaultTableModel(columns, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return false;
        ...
        for (BorrowRequest request : requests) {
            ...
            String borrowedAt = transaction == null ? "Pending approval" : formatDate(transaction.getBorrowDate());
                String dueDate = transaction == null || transaction.getDueDate() == null
                    ? request.getDueDate() == null ? "-" : formatDate(request.getDueDate())
                    : formatDate(transaction.getDueDate());
            String returnedAt = transaction == null || transaction.getReturnDate() == null
                    ? "Not yet returned" : formatDate(transaction.getReturnDate());
            String status = transaction == null ? request.getStatus() : "Successful";
                model.addRow(new Object[]{bookName, request.getBorrowerName(), borrowedAt, dueDate, returnedAt, status});
        }
    ...
    private void showBorrowBookDialog() {
        ...
        for (Book book : books) {
            if (book.isAvailable() && !isBookOccupied(book.getBookID())) {
                availableBooks.add(book);
            }
        }

        if (availableBooks.isEmpty()) {
            outputArea.setText("No books are currently available. All books are occupied or unavailable.");
            return;
        }
        ...
        JLabel dueDateLabel = new JLabel("Return date (YYYY-MM-DD):");
        JTextField dueDateField = new JTextField();
        form.add(bookLabel);
        form.add(bookSelector);
        form.add(nameLabel);
        form.add(nameField);
        form.add(dueDateLabel);
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            try {
                ...
                dueDate = dateFormat.parse(dueDateField.getText().trim());
                if (!dueDate.after(new Date())) {
                    outputArea.setText("Return date must be later than today.");
                    return;
                }
            } catch (java.text.ParseException ex) {
                outputArea.setText("Please enter a valid return date using YYYY-MM-DD.");
                return;
            }
            Book selectedBook = availableBooks.get(bookSelector.getSelectedIndex());
            outputArea.setText(requestBorrow(selectedBook.getBookID(), borrowerName, dueDate));
            showBorrowRecords();
        }
    }
    ...
    // Gumagawa ng pending request; ang transaction ay ginagawa lamang kapag na-approve.
    private String requestBorrow(String bookID, String borrowerName, Date dueDate) {
        if (!(currentUser instanceof Student)) {
        ...
        if (!selectedBook.isAvailable() || isBookOccupied(bookID)) {
            return "Selected book is currently unavailable.";
        }

        for (BorrowRequest existingRequest : requests) {
            if (existingRequest.getStudentID().equalsIgnoreCase(currentUser.getUserID())
                    && existingRequest.getBookID().equalsIgnoreCase(bookID)
                    && "Pending".equalsIgnoreCase(existingRequest.getStatus())) {
                return "This book already has a pending borrow request.";
            }
        }

        String requestID = "REQ" + (requests.size() + 1);
        Date borrowDate = new Date();
        BorrowRequest request = new BorrowRequest(requestID, currentUser.getUserID(), borrowerName,
            bookID, borrowDate, dueDate, "Pending");
        requests.add(request);
        selectedBook.setAvailable(false);
        refreshBookAvailabilityLabels();

        notifications.add(new Notification("N" + (notifications.size() + 1),
            "Borrow request pending | Book: " + selectedBook.getTitle()
                + " | Borrower: " + borrowerName,
            new Date(), "Sent"));
        return "Borrow request submitted for admin approval.\nBook: " + selectedBook.getTitle()
            + "\nBorrower: " + borrowerName
    ...
    private String approveRequest(String requestID) {
        for (BorrowRequest request : requests) {
            if (request.getRequestID().equalsIgnoreCase(requestID)) {
                ...
                if (findTransaction(request.getStudentID(), request.getBookID()) != null) {
                    return "A borrow record already exists for this request.";
                }
                request.setStatus("Approved");
                Book book = findBook(request.getBookID());
                if (book == null) {
                    return "Book not found for this request.";
                }
                book.setAvailable(false);
                refreshBookAvailabilityLabels();
                BorrowTransaction transaction = new BorrowTransaction("T" + (transactions.size() + 1),
                        request.getBookID(), request.getStudentID(), request.getBorrowerName(), new Date(),
                    request.getDueDate(), "Active");
                transactions.add(transaction);
                request.setStatus("Successful");
                User borrower = users.get(request.getStudentID());
                if (borrower instanceof Student) {
                    ((Student) borrower).borrowBook(request.getBookID());
                }
                notifications.add(new Notification("N" + (notifications.size() + 1),
                    "Borrow approved | Book: " + book.getTitle()
                        + " | Borrower: " + request.getBorrowerName()
                        + " | Borrowed at: " + formatDate(transaction.getBorrowDate())
                        + " | Return by: " + formatDate(transaction.getDueDate()),
                    new Date(), "Sent", request.getStudentID()));
                return "Borrow successful and book marked as borrowed.\nReturn by: "
                    + formatDate(transaction.getDueDate());
            }
        }
    ...
    private void checkStudentOverdueNotifications(Student student) {
        List<String> overdueMessages = new ArrayList<>();
        Date now = new Date();
        for (BorrowTransaction transaction : transactions) {
            if (!transaction.getBorrowerID().equalsIgnoreCase(student.getUserID())
                    || !"Active".equalsIgnoreCase(transaction.getStatus())
                    || transaction.getDueDate() == null
                    || !transaction.getDueDate().before(now)) {
                continue;
            }

            Book book = findBook(transaction.getBookID());
            String bookName = book == null ? transaction.getBookID() : book.getTitle();
            String message = "OVERDUE | Book: " + bookName
                    + " | Due date: " + formatDate(transaction.getDueDate());
            boolean alreadySent = false;
            for (Notification notification : notifications) {
                if (notification.getRecipientID().equalsIgnoreCase(student.getUserID())
                    && notification.getMessage().startsWith(message)) {
                    alreadySent = true;
                    break;
                }
            }
            if (!alreadySent) {
                notifications.add(new Notification("N" + (notifications.size() + 1),
                        ...
                        now, "Overdue Notice Sent", student.getUserID()));
        ...
        if (!overdueMessages.isEmpty()) {
            JOptionPane.showMessageDialog(this,
                    String.join("\n", overdueMessages) + "\n\nPlease return the overdue book.",
                    "Library Information System Notification", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private String processReturn(String transactionID) {
        for (BorrowTransaction transaction : transactions) {
            if (transaction.getTransactionID().equalsIgnoreCase(transactionID)) {
                if (!"Active".equalsIgnoreCase(transaction.getStatus()) || transaction.getReturnDate() != null) {
                ...
                User borrower = users.get(transaction.getBorrowerID());
                if (borrower instanceof Student) {
                    ((Student) borrower).returnBook(transaction.getBookID());
                }
